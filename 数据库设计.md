...
系统主要包含以下核心实体：
- **User (用户)**: 包含买家、卖家和管理员信息，集成钱包账户。
- **Item (拍品)**: 拍卖的商品信息，关联卖家、分类、和最终中标者。
...
- **WalletTransaction (资金明细)**: 用户的充值、支付、退款流水。
- **Deposit (保证金)**: 竞拍特定商品的保证金冻结记录。
- **Favorite (收藏)**: 用户对商品的关注记录。
...
| `email` | VARCHAR(120) | NULLABLE | 用户邮箱（替代手机号） |
| `avatar` | VARCHAR(200)| NULLABLE | 用户头像文件名 |
| `banned_until` | DATETIME | NULLABLE | 封禁截止时间 |
| `real_name` | VARCHAR(80)| NULLABLE | 真实姓名（实名认证） |
| `id_card` | VARCHAR(20) | NULLABLE | 身份证号（实名认证） |
| `is_verified` | BOOLEAN | DEFAULT FALSE | 是否已实名认证 |
| `verified_at` | DATETIME | NULLABLE | 实名认证通过时间 |
| `wallet_balance` | DECIMAL(10,2)| DEFAULT 0.00 | 可用余额 |
| `wallet_frozen` | DECIMAL(10,2)| DEFAULT 0.00 | 冻结余额（如保证金/支付中） |
...
| `category` | VARCHAR(50)| NULLABLE | 商品分类（如：艺术品、电子产品） |
...
| `status` | VARCHAR(20) | DEFAULT 'pending' | 状态：`pending`, `active`, `rejected`, `ended`, `stopped` |
...
| `payment_status` | VARCHAR(20) | DEFAULT 'unpaid' | 支付状态：`unpaid`, `paid`, `timeout_cancelled` |
...
| `shipped_at` | DATETIME | NULLABLE | 发货时间 (用于计算自动确认收货) |
| `shipping_extended_count` | INT | DEFAULT 0 | 延长收货次数 |
...
| `seller_id` | INT | FOREIGN KEY (users.id) | 卖家ID |
| `last_message` | VARCHAR(255)| NULLABLE | 最后一条消息预览 |
...
| `status` | VARCHAR(20) | DEFAULT 'frozen' | 状态：`frozen`, `applied`, `refunded`, `forfeited` |
| `created_at` | DATETIME | DEFAULT CURRENT_TIMESTAMP | 创建时间 |

### 2.10 资金明细表 (`wallet_transactions`)
用户钱包的所有资金变动流水。
...
| `user_id` | INT | FOREIGN KEY (users.id) | 用户ID |
| `item_id` | INT | FOREIGN KEY (items.id) | 关联拍品ID (可选) |
| `type` | VARCHAR(30) | NOT NULL | 类型：`recharge`, `deposit`, `refund`, `payment`, `forfeit`, `payout` |
| `direction` | VARCHAR(10) | NOT NULL | 方向：`credit` (入账), `debit` (出账) |
| `amount` | DECIMAL(10,2)| NOT NULL | 变动金额 |
| `balance_after`| DECIMAL(10,2)| NOT NULL | 变动后余额 |
| `description` | VARCHAR(255)| NULLABLE | 备注说明 |
...
10. **Item (1) -> (N) Appeal**: 一个拍品可能有多次申诉记录。
11. **User (1) -> (N) WalletTransaction**: 一个用户拥有多条资金流水。
12. **User (1) <-> (N) Item (Favorites)**: 多对多关系，用户可收藏多个拍品。
...
- `wallet_transactions.user_id`: 钱包页面查询个人流水。
- `deposits.item_id` + `deposits.user_id`: 快速校验用户是否已交保证金。
